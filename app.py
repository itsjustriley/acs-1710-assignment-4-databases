...
def plants_list():
    ...
    plants_data = mongo.db.plantsDatabase.find()
...
def create():
    ...
    if request.method == 'POST':
        ...
        result = mongo.db.plantsDatabase.insert_one(new_plant)
...
def detail(plant_id):
    ...
    plant_to_show = mongo.db.plantsDatabase.find_one({'_id': ObjectId(plant_id)})
    ...
    # collection.
    harvests = mongo.db.harvestDatabase.find({'plant_id': plant_id})
...
def harvest(plant_id):
    ...
    mongo.db.harvestDatabase.insert_one(new_harvest)
...
def edit(plant_id):
    ...
    if request.method == 'POST':
        ...
        mongo.db.plantsDatabase.update_one({'_id': ObjectId(plant_id)}, {'$set': {
            'name': name,
            'variety': variety,
            'photo_url': photo_url,
            'date_planted': date_planted
...
def delete(plant_id):
    ...
    # id.
    mongo.db.plantsDatabase.delete_one({'_id': ObjectId(plant_id)})

    # TODO: Also, make a `delete_many` database call to delete all harvests with
    # the given plant id.
    mongo.db.harvestDatabase.delete_many({'plant_id': plant_id})
